...
export async function run(): Promise<void> {
  try {
    ...
    const generateCoverage =
      core.getInput('generate-coverage').toLowerCase() === 'true'
    const generateQuality =
      core.getInput('generate-quality').toLowerCase() === 'true'
    const generateQodanaButton =
      core.getInput('generate-qodana-button').toLowerCase() === 'true'
